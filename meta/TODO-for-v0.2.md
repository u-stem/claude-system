...
## 18. `git tag v0.1.0` の発行可否(運用者判断待ち)

### 経緯

[ADR 0005](./decisions/0005-bootstrap-completion-and-deferral.md) が「Phase 10 完了時に `git tag v0.1.0`」と約束したが未発行のまま 3 か月経過し、2026-08-09 の棚卸しで発見された。決着方針は [ADR 0025](./decisions/0025-symlink-switchover-record-and-release-tagging.md) に記録。

### 判断が要る点

- タグを打つ対象コミット: 現 HEAD か、切り替え当日(2026-05-04)のコミットか。現 HEAD は bootstrap 完了時点から大きく先行しており、`v0.1.0` が指す内容は ADR 0005 の想定と別物になる
- `VERSION` ファイルは bootstrap 以来 `0.1.0` のまま。タグを打つならファイルとタグが初めて一致する。打たないなら次のバンプ時に `0.2.0` へ進め、`v0.1.0` は永久欠番として扱う判断もありうる

### なぜ自動で実行しないか

タグ発行は push を伴う不可逆・外向き操作。[ADR 0009](./decisions/0009-opus-48-autonomy-tuning.md) の線引きにより運用者の明示判断を要する。
