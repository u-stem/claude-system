...
宣言と実体が別管理のため、明示的に実行する。**`settings.json` の `enabledPlugins` は宣言にすぎず、これだけではプラグインは入らない**(この誤解で 3 件が 3 か月以上「宣言のみ」だった / [ADR 0023](./decisions/0023-harness-sync-2.1.226.md))。
