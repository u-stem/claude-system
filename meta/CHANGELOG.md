...
1. 提案したゲートは、動機として引用した 4 つの実失敗を**1 つも止められない**(当時の要求証跡はすべて充足していた)。しかも計画自身が「doc 追従の欠落のみ → 警告のみ」と書いており、動機となった失敗クラスを唯一ブロックしない層に分類していた
2. **Stop hook は「完了」を観測できない**。実測でログ 1602 行・当日 48 回発火しており、入力待ちに戻るたびに起きる。状態で判定すれば作業途中の質問が誤爆し、メッセージ内容で判定すれば「完了と書かなければ回避できる」
3. 手続きの証跡を要求するゲートは**儀式化して実質を損なう**。見出しを書き doctor を 1 回叩き reviewer を 1 回起動すれば通り、「ゲートを通った = 品質が担保された」という誤った安心を生む

運用者の実問題は完了判定の甘さではなく**手戻り**(「作業中に出戻りや、後からこれがだめだったが何度も発生する」)だと確認できたため、ADR 0024 の結論(誰が起動しても効く層は git 自身)に倣い、**実害の返済 → git 層での再発防止 → 手戻りの計測**に組み替えた。介入の設計は実データを見てから決める(ADR 0019「計測なき格上げをしない」)。

**実害の返済**

- Phase 10(2026-05-04 完了)が未反映だった 16 ファイルを是正。最も実害があったのは `doctor.sh` で、`~/.claude/<sub>` が symlink でない状態を `ok (informational)`、`~/.claude` 不在を `ok ... expected pre-Phase 10` と報告していた。**一度も sync していないマシンを健全と判定する**状態だったため、両者を警告に変更。`meta/multi-device-setup.md` も同じ反転を抱えていた
- `tools/sync.sh` の `CLAUDE_SYSTEM_ALLOW_SYNC` は**撤去せず根拠を差し替えた**。ガード自体は正当(ライブの `~/.claude/` を書き換える / `CS_ALLOW_PUSH` と同型)で、古いのは「Phase 0-9 だから」という理由付けの方だった
- `tools/disable-guardrails.sh` / `enable-guardrails.sh` に `--dry-run`(新規テスト 15 ケース)。緊急停止経路そのものが観測不能な状態の解消。未知引数を exit 2 で拒否するようにし、`--dryrun` の打ち間違いが本番実行になる経路も塞いだ
- ルート README の多層防御表を是正。4 行目「Stop hooks / 失敗未解決での Stop を阻止」は**一度も実装されたことのない虚偽広告**だった(当初この行を「設計意図の前例」として引用しかけたが、反証で誤りと判明)。git pre-push 層を追加し、`core.hooksPath` 未設定なら push ガードごと消えることを明記
- [ADR 0025](./decisions/0025-symlink-switchover-record-and-release-tagging.md) 起票。ADR 0005 が約束した「`0006-symlink-switchover.md` 起票」と「`git tag v0.1.0`」が 3 か月未履行だった件を決着。0006 番は同日別テーマへ採番済みのため番号を繰り上げ、切り替えの実施記録を引き継いだ。**タグ発行は不可逆・外向き操作のため実行せず**、選択肢を添えて TODO 項目 18 に記録
- 真因は「ADR に未来の実行の約束を書いたが、回収する仕組みと接続されていなかった」こと。`meta/decisions/README.md` に**TODO への転記規約**を新設し、ADR 0025 自身の保留事項に最初に適用した
- ADR Status 語彙に `Partially superseded by NNNN` を追加(全 24 本が `Accepted` で現行方針を判別できなかった。単純な `Superseded` では 0010/0016 に残る有効な判断まで無効に見える)。0014 の語彙外 Status も正規化し、全 25 本が語彙内になった

**git 層での再発防止**

- `tests/check-doc-parity.sh`(新規、テスト 17 ケース): スクリプトが索引 README に載っているか / 失効した Phase 表記 / ADR Status の語彙と相互参照、の 5 検査。**新規 hook 0 本**で、CI と `githooks/pre-push` の既存経路にそのまま乗る。手作業で見落としていた 3 件(settings template の Phase 10 記述 / `notify-stop-failure.sh` の未記載 / ADR 0010 の片方向参照)を即座に検出した
- 意図的に textual に留めた。「言及がない」「表記が失効した」は検出できるが記述の正確さは判定できない。偽陽性ゼロを最優先し(偽陽性を出す検査は無効化され、無効化された検査は何も守らない)、テストの第一ケースが実リポジトリでの pass を assert する
- 検査が要求した既存不整合を解消: `tests/README.md`(bootstrap 以来未更新、稼働 15 本が 0 本記載)を全面書き換え / `tools/README.md` に 2 本追加 / `hooks/_README.md` の event 誤記と欠落を是正
- `tools/githooks/*` を shellcheck 対象に追加。`.sh` 拡張子がないためどのグロブにも掛からず、**pre-push を含めて全ゲートの外**にあった
- CI の重複 lint ステップを削除し、代わりに `test-pre-bash-guard.sh` を追加。ローカル push 時のみ走っており GitHub 側で回帰を検出できなかった
- 文字数カウントのロケール依存を解消(`cs_utf8_locale` / `cs_str_chars`)。`en_US.UTF-8` 不在の CI イメージでは `wc -m` がバイト数を返し、16 skill 中 12 件が誤って「50 字超」になる無音の失敗モードがあった

**観測系の欠陥修正**

- セッション開始時の `[test] 3 failures` は**すべて過去ログの再掲**だった(2 件は 2026-07-11 の TDD レッド段階、1 件はガードを意図的に壊した negative test の副産物)。出力に判別材料がなく、本セッションの調査を一度誤らせた
- `log-failure.sh` に `intent` フィールド、`check-failure-patterns.sh` に直近 14 日窓 + `intent:expected` 除外 + **日付範囲の常時表示**。集計を `grep -c` から `jq -R 'fromjson?'` に変更し、壊れた行で hook が止まらないようにした。詳細は [ADR 0020 の Update](./decisions/0020-failure-hook-event-migration.md)
- 検証: 既定窓では出力なし(誤警告が消えた)、90 日窓では同じ 3 件が `(2026-07-11 .. 2026-08-09)` として現れ、古い記録だと一目で分かる。データは失われていない

**手戻りの計測(何もブロックしない)**

- `record-rework-signal.sh`(新規、PostToolUse(Edit|Write)、テスト 11 ケース): 編集ファイルを `.claude/rework-log.jsonl` に 1 行記録するだけ。パスはプロジェクト相対(絶対パスは ADR 0008 が禁じる username を含むため)、あらゆる異常系で exit 0
- `session_id` と `agent_type`/`agent_id` を**両方**記録する。subagent の PostToolUse payload がメインの session_id を運ぶかは未検証で、事前に測る手段もなかった(記録している hook が存在しなかった)。異なる場合、session_id だけで集約すると**委譲ファーストを守ったセッションほど手戻りを過小評価する**。ADR 0024 が `agent_type` で採った「仮定せず記録して後で決める」手順を踏襲
- `loop-report.sh` に rework セクション(セッションあたり編集数 / 同一セッションで 3 回以上編集されたファイル / agent_type 別内訳)と、failure-log の intent 別内訳を追加
- **意図的に何も判定していない**。介入の要否は「手戻りが多いセッションで計画・反証段が飛ばされていたか」の相関で決める。それは実データの問題であり、現時点の正直な答えは「まだ分からない」

**この作業が解決していないこと**

- 「本来 Superseded にすべき ADR が Accepted のまま」は検出できない(何が置き換えられたかの意味理解が要る)。ADR 本文の約束の未履行も同様で、そこは §4 の転記規約という人間側の接続点でカバーする
- gitleaks の履歴モード(2026-08-09 の CHANGELOG が「3 か月すり抜けた原因」と自己分析した項目)は**未実施**。`secrets-scan.yml` が権限設定で読めず、変更も検証もできなかった
- `--dry-run` 未実装のスクリプトが 7 本残っている(優先度低)
