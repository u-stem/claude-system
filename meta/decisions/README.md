...
| 自前 skill / command は使用実績で剪定する(180 日 0 回は削除候補)。frontmatter は `name` / `description` のみ | `/skill-doctor` で 13 件が 0 回だった | 2026-12 の update-check の `/skill-doctor` で `nextjs-supabase-base` / `nextjs-supabase-rls` / `project-tech-stack-decision` / `review-loop` / `team` が引き続き 0 回なら削除(2026-09-23 は 17 日窓のため維持)。`doc-writer` subagent(5 回、直近 30 日 0)も同時判定 | 全件維持、`nextjs-supabase-*` を `project-templates/nextjs-supabase/.claude/skills/` へ移して listing コストを 0 にする案(削除時の代替として保持) | 0027 / 注記: CHANGELOG 2026-09-23 |
