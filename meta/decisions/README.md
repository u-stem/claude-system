...
| [0010](./0010-opus-48-harness-settings-sync.md) | Opus 4.8 Harness Settings Synchronization | Partially superseded by 0016 (2026-05-29) | ADR 0009 の機械層同期。`settings.json.template` の model pin を 4.8 に、`VERSION` を実インストール版(2.1.156)に同期。autonomy 方針は文脈依存判断のため hook 強制せず、既存 deny/ask ガードが「不可逆操作は確認」の線引きを部分担保していることを確認・記録 |
...
| [0026](./0026-harness-sync-2.1.229.md) | Harness Sync 2.1.229 — 検証手段の是正と、宣言系統に残るもう 1 つの乖離 | Accepted (2026-08-13) | VERSION 2.1.226→2.1.229。当初計画の hook 検証が「証明にならない」と反証で判明し、受動観測から**能動負テスト**へ差し替え(subagent の `git push --dry-run` が deny され `agent_type: Explore` を実測。合成 payload のテストでは代替できない)。`enabledPlugins` と同型の乖離が **MCP 宣言系統**に残ることを実測(`setup-mcp.sh` は版を見ない / `doctor.sh` は構文しか見ない)— opt-in は未登録が正常状態のため塞がず記録。`check-doc-parity.sh` に VERSION↔README 検査を追加(過去 2 回ズレた対を機械化)。`chrome-devtools-mcp` 1.7.0 は公開 3 日で 7 日ルール抵触のため見送り。Betterleaks を ADR 本文から TODO 項目 19 へ転記し先送りを可視化 |
