...
2. **グローバル `git config` の `user.name` / `user.email` が暗黙に commit author として使われそうになった**
   - `user.name = u-stem`(GitHub handle)
   - `user.email = tanaka128821@gmail.com`(personal email)
...
その後オーナー自身が追加判断として、過去の Public リポジトリ(sugara, kairous, mc-server, u-stem, inspiration, sugara-releases, selfos-open の 7 件)で既に `tanaka128821@gmail.com` が commit author として露出済みであることを確認し、claude-system だけ noreply 化しても防御線として機能しないため、例外的に personal email の commit author 使用を許容することとした(同時に GitHub handle `u-stem` を LICENSE Copyright holder にも採用)。
...
| GitHub handle (`u-stem`) | **許容** | LICENSE, README, ドキュメント, commit author 等で使用可 |
| Personal email (`tanaka128821@gmail.com`) | **条件付き許容** | commit author 等、既に他 Public リポジトリで露出済みの用途は許容。README やドキュメントの本文として「新たに」露出を増やす書き方はしない |
...
- LICENSE の Copyright holder は GitHub handle `u-stem` を採用(本 ADR 採択と同時に決定、初期コミット `42caea3` に取り込み済み)
...
- `meta/TODO-for-phase-3.md`(Phase 3 で principles 層 / adapters 層に取り込む)
